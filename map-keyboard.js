(() => {
  ...
  if (typeof state !== 'object' || typeof map === 'undefined') return;
  ...
  function recordsForMarker(marker) {
    const records = [];
    state.markers.forEach((candidate,id) => {
      if (candidate !== marker) return;
      const record = state.adventures.find(item => item.id === id);
  ...
  function decorateArchive() {
    document.querySelectorAll('.adventure-item').forEach(button => {
      const active = Boolean(state.focusId) && button.dataset.id === state.focusId;
