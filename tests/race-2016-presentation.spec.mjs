...
test('2016 Heartland and Illinois grouped race stories render the intended structures', async ({ page }) => {
  ...
  await expect(series.locator('.series-year-grid')).toContainText('1:47:58');
  await expect(series).toContainText('1:43:16');
  await expect(series.locator('.series-year-grid')).toContainText('1:43:29');
  await expect(series.locator('.series-performance')).not.toBeVisible();
  await series.locator('summary').click();
  await expect(series.locator('.series-performance')).toBeVisible();
...
test('2016 race memories render on their canonical race records', async ({ page }) => {
  ...
  await expect(page.locator('.race-memory-finish strong')).toHaveText('1:47:58');
