...
for (const [path, label] of sections) {
  test(`${label} clean page keeps the shared Adventures shell consistent`, async ({ page }) => {
    ...
    if (label !== 'Map') await expect(page.locator('footer')).toContainText('Alex Ford Adventures');
...
test('Explore chapter cards keep their source targets aligned with the clean published routes', async ({ page, request }) => {
