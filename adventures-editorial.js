(()=>{
  ...
  function renderLead(){
    ...
    const a=records[0];
    host.innerHTML=`<a class="story-cover" href="${A.recordHref(a)}"><div class="story-cover-copy"><p class="story-cover-kicker">Latest story · ${esc(type(a))}</p><h2>${esc(a.name)}</h2><p class="story-cover-dek">${esc(dek(a))}</p><div class="story-cover-meta"><span>${esc(dateLine(a))}</span><span>${esc(a.location||a.region||'')}</span><span>${esc(metric(a))}</span></div><span class="story-read">Read the story →</span></div><aside class="story-cover-folio" aria-hidden="true"><small>Adventures</small><strong>${String(records.length).padStart(2,'0')}</strong><span>${esc(a.region||'Field notes')}</span></aside></a>`;
  }
  function renderShelf(){
    const host=document.getElementById('storyShelf');if(!host)return;
    const picks=records.slice(1,4);
    host.innerHTML=picks.map((a,i)=>`<a class="story-feature-card story-feature-${i+1}" href="${A.recordHref(a)}"><div><p class="story-feature-kicker">${esc(type(a))} · ${esc(dateLine(a))}</p><h3>${esc(a.name)}</h3><p>${esc(dek(a))}</p></div><div class="story-feature-foot"><span>${esc(a.location||'')}</span><strong>${esc(metric(a))}</strong></div></a>`).join('')||'<p class="empty">More stories will appear here as the collection grows.</p>';
  }
