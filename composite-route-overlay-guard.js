(() => {
  ...
  routes.compositeRouteColor = (feature, context) => {
    // Route styling resolves before Leaflet commits the SVG stroke. Mark the
    // page at that exact point so the shared !important accent rule cannot
    // repaint a composite member route after its per-route color is chosen.
    // Single-route pages never receive a composite context and keep the
    // existing accent behavior.
    if (context?.recordId && context?.members?.length) {
      document.body?.classList.add('has-composite-routes');
    }
