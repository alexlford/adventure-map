...
const mapRouteDetail=fs.readFileSync('map-route-detail.js','utf8');
const mapUrlState=fs.readFileSync('map-url-state.js','utf8');
...
const apiScript='<script src="adventure-map-api.js"></script>';
const routeDetailScript='<script src="map-route-detail.js"></script>';
const urlStateScript='<script src="map-url-state.js"></script>';
const apiIndex=mapPage.indexOf(apiScript),routeDetailIndex=mapPage.indexOf(routeDetailScript),urlStateIndex=mapPage.indexOf(urlStateScript);
if(routeDetailIndex<0)errors.push('map page must load lazy route-detail behavior through map-route-detail.js');
if(apiIndex<0||routeDetailIndex<=apiIndex)errors.push('map route-detail behavior must load after the stable AdventureMap API');
if(apiIndex<0||urlStateIndex<=apiIndex)errors.push('map URL state must load after the stable AdventureMap API');
if(routeDetailIndex>=0&&urlStateIndex>=0&&urlStateIndex<=routeDetailIndex)errors.push('map URL state must load after the lazy route-detail runtime extension');
...
for(const method of ['ready','record','filteredRecords','visibleRoutes','popupHtml','focus','emphasize','clearFocus','fit','refresh','setViewState'])if(!mapApi.includes(`${method}(`)&&!mapApi.includes(`${method},`))errors.push(`AdventureMap API missing ${method}`);
if(!mapRouteDetail.includes('const runtime = window.AdventureMapRuntime')||!mapRouteDetail.includes('AdventureRoutes.loadDetailForAdventure'))errors.push('map route-detail behavior must operate through the internal AdventureMap runtime and addressable route loader');
if(!mapUrlState.includes('const api = window.AdventureMap')||!mapUrlState.includes('api.ready()'))errors.push('map URL state must operate through the AdventureMap API');
if(mapUrlState.includes('MutationObserver'))errors.push('map URL state must not use DOM observation for readiness');
if(/initYearControls\s*=/.test(mapUrlState))errors.push('map URL state must not monkey-patch initYearControls');
for(const legacyCall of ['filteredAdventures(','focusAdventure(','publicLayerFor(','render()'])if(mapUrlState.includes(legacyCall))errors.push(`map URL state still calls legacy map global ${legacyCall}`);
...
console.log(`Canonical route-registry validation passed for ${siteRoutes.length} public routes with API-based map URL state.`);
