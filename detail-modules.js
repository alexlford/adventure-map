window.AdventureDetailModules = (() => {
  const A = window.AdventureSite;
  if (!A) throw new Error('AdventureSite is required for detail modules');
  const esc = A.esc;
  ...
  function typeDossier(a, all, rels) {
    if (a.kind === 'race') {
    ...
    if (a.kind === 'outing' && (a.discipline === 'mountain-bike' || a.discipline === 'nordic')) {
  ...
  function storyModule(a, all, relationships, mediaHtml = '') {
    if (a.kind !== 'adventure') return null;
    ...
    const companions = (Array.isArray(a.companions) ? a.companions : []).filter(item => item && item.name);
  ...
  const splitRows = record => (record.officialSplits || []).map(s => `<tr><th scope="row">${esc(s.label)}</th><td>${esc(s.time || '—')}</td><td>${esc(s.segmentTime || '—')}</td><td>${esc(s.pace || '—')}</td></tr>`).join('');
  ...
  function raceArc(record) {
    const splits = (record.officialSplits || []).map(s => ({ ...s, paceSeconds: paceSeconds(s.pace) })).filter(s => Number.isFinite(s.paceSeconds));
  ...
  function worldMajorModule(record, data, mediaHtml = '') {
    const major = (data?.majors || []).find(x => x.recordId === record.id);
    if (!major || major.status !== 'completed') return '';
    const completed = (data.majors || []).filter(x => x.status === 'completed'), order = completed.findIndex(x => x.id === major.id) + 1;
    const result = record.officialTime || A.formatDuration(record.elapsedSeconds) || 'Recorded', course = available(record, 'course'), splits = record.officialSplits || [], photos = hasPhotos(record), story = hasStory(record);
    ...
    const assetLine = `${available(record, 'result') ? '✓ Result' : '＋ Result'} · ${splits.length ? '✓ Splits' : '＋ Splits'} · ${course ? '✓ Course' : '＋ Course'} · ${photos ? '✓ Photos' : '＋ Photos'} · ${story ? '✓ Story' : '＋ Story'}`;
