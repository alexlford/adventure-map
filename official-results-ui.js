(()=>{
  ...
  popupCard=function(a){
    const category=CATEGORY[publicLayerFor(a)]||{label:'Adventure'};
    if(a.kind!=='race'){
      ...
      const metrics=a.distanceMi?`<p class="popup-meta">${escapeHtml(a.distanceMi)} mi${showGain&&a.elevationGainM?` · ${escapeHtml(Math.round(a.elevationGainM))} m gain`:''}${a.elapsedSeconds?` · ${escapeHtml(formatDuration(a.elapsedSeconds))} elapsed`:''}</p>`:'';
      return `<article class="popup-card"><p class="popup-kicker">${escapeHtml(category.label)}</p><h3 class="popup-title">${escapeHtml(a.name)}</h3>${alias}<p class="popup-meta">${escapeHtml(primary)}${primary?' · ':''}${escapeHtml(a.location)}</p>${date}${metrics}<p class="popup-detail"><a href="${recordHref(a)}">Open record →</a></p></article>`;
    }
