(() => {
  ...
  const deltaLabel = delta => {
    ...
    return `${formatClock(Math.abs(delta))} ${direction} than prior appearance`;
  };
  ...
  const chartFor = members => {
    ...
    const rows = entries.map((item, index) => {
      ...
      const delta = Number.isFinite(previous) ? item.time.seconds - previous : null;
      return `<div class="series-chart-row${isFastest ? ' is-fastest' : ''}">
        <div class="series-chart-label"><strong>${A.esc(String(item.record.year || item.record.date?.slice(0, 4) || '—'))}</strong><span>${A.esc(item.time.label)}</span></div>
        <div class="series-chart-track" aria-hidden="true"><span style="--series-bar:${width.toFixed(1)}%"></span></div>
        <div class="series-chart-meta"><span>${A.esc(index ? deltaLabel(delta) : item.time.sourceLabel)}</span>${isFastest ? `<em>${A.esc(fastestLabel)}</em>` : ''}</div>
      </div>`;
    }).join('');
  ...
  const cardFor = record => {
    ...
    const route = hasGpsRoute(record);
    const year = record.year || record.date?.slice(0, 4) || '—';
    const result = time.source === 'official' ? time.label : time.source === 'gps' ? time.label : 'Result pending';
    return `<a class="series-year-card" href="${A.recordHref(record)}">
      <div class="series-year-top"><span>${A.esc(String(year))}</span>${route ? '<em>GPS course</em>' : '<em>Location only</em>'}</div>
      <small>${A.esc(distanceLabel(record))}</small>
      <strong>${A.esc(result)}</strong>
      <p>${A.esc(time.sourceLabel)}</p>
      <footer><span>${A.esc(record.name)}</span><b>Open record →</b></footer>
    </a>`;
  };
  ...
  async function enhance() {
    ...
    const [recordPayload, relationshipPayload] = await Promise.all([
      fetchJson('data/public-records.json'),
    ...
    if (!record || record.kind !== 'adventure') return;

    const relationship = relationships.find(rel => rel.type === 'series' && rel.adventureId === record.id && Array.isArray(rel.memberIds) && rel.memberIds.length > 1);
    ...
    section.innerHTML = `<div class="race-series-head"><div><p class="eyebrow">Recurring race series</p><h2>${A.esc(relationship.name || record.name)}</h2></div><p>${A.esc(relationship.summary || record.note || 'A multi-year race history preserved as one connected series.')}</p></div>
      <div class="race-series-stats">
        <article><small>Appearances</small><strong>${members.length}</strong><span>${A.esc(span)}</span></article>
        <article><small>Cumulative race distance</small><strong>${A.esc(mileage)}</strong><span>Organizer distance when available</span></article>
        <article><small>Official results</small><strong>${officialCount}</strong><span>${members.length - officialCount} GPS/fallback records</span></article>
        <article><small>Course archive</small><strong>${gpsCount}/${members.length}</strong><span>${gpsCount > 1 ? 'Routes overlay on the map below' : 'GPS routes attached when available'}</span></article>
      </div>
      ${chartFor(members)}
      <div class="series-year-grid">${members.map(cardFor).join('')}</div>
      ${gpsCount > 1 ? `<div class="series-course-callout"><div><p class="eyebrow">Course evolution</p><h3>${gpsCount} recorded courses, one map.</h3></div><p>The course map below overlays the available personal GPS routes with a separate color for each appearance. Zoom in to compare the shared sections, reroutes, start/finish changes, and event-to-event course drift.</p></div>` : ''}`;

    editorial.insertAdjacentElement('afterend', section);
    const genericChallenge = page.querySelector('.story-objective-feature.challenge-feature');
    if (genericChallenge) genericChallenge.remove();
  ...
  let queued = false;
  const queueEnhance = () => {
    if (queued || page.dataset.raceSeriesEnhanced === 'true') return;
    queued = true;
