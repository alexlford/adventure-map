(()=>{
  ...
  const mountainLoopFeature=(a,summits)=>{
    ...
    const stats=[
      Number.isFinite(a.distanceMi)?['Loop distance',`${a.distanceMi} mi`]:null,
      gainFt?['Recorded gain',`${A.fmt.format(gainFt)} ft`]:null,
      Number.isFinite(a.elapsedSeconds)?['Elapsed',A.formatDuration(a.elapsedSeconds)]:null,
  ...
  Promise.all([A.load(),A.loadRelationships()]).then(([all,relationships])=>{
    ...
    const relatedFromRelationships=rels.flatMap(r=>(r.memberIds||[]).map(id=>byId.get(id))).filter(Boolean);
    const relatedSummits=(a.linkedSummits||[]).map(id=>byId.get(id)).filter(Boolean);
    const genericConnected=uniq(relatedFromRelationships).filter(x=>x.id!==a.id&&!relatedSummits.some(s=>s.id===x.id));
    const allConnected=uniq([...genericConnected,...relatedSummits]).filter(x=>x.id!==a.id);
    const mountainFeature=mountainLoopFeature(a,relatedSummits);
    const connections=mountainFeature?`${mountainFeature}${genericConnected.length?genericConnections(genericConnected):''}`:genericConnections(allConnected);
