window.AdventureSite = (() => {
  ...
  const raceType = (a) => a.discipline === 'marathon' ? 'Marathon' : a.discipline === 'trail' ? 'Trail race' : a.discipline === 'nordic' ? 'Nordic ski race' : a.discipline === 'relay' ? 'Relay' : a.kind === 'race' ? (a.distance || 'Road race') : adventureType(a);
  ...
  async function load() {
    ...
    const merged=base.adventures.map(a=>({...a,...(strava.matches?.[a.id]||{})})); const seen=new Set(merged.map(a=>a.id));
